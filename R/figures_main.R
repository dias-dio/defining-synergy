...
plot_freq_with_meanzip_labels <- function(dat, y_max, angle = 45) {
  ...
  ggplot(dat2, aes(x = pathway, y = n_hits)) +
    ...
    theme(
      ...
      panel.grid.major.y = element_line(colour = "grey75", size = 0.35),
      panel.grid.minor = element_blank(),
...
#### Supplementary Figure 10 - Target pathway prioritization under Benjamini-Hochberg (BH) false discovery rate (FDR)–controlled empirical significance
